<?php
/* ============================================================
   supervisor/index.php — LOGIN PORTAL SUPERVISOR
...
// Si ya hay sesión activa de supervisor → redirigir
if (
    (!empty($_SESSION['supervisor_portal_rol']) && $_SESSION['supervisor_portal_rol'] === 'supervisor') ||
    (!empty($_SESSION['usuario_id']) && ($_SESSION['rol'] ?? '') === 'supervisor')
) {
...
try {
    ...
    error_log('supervisor/index.php: ' . $e->getMessage());
}
    ...
    if (empty($usuario) || empty($password)) {
        ...
        try {
            ...
            if (!$user) {
                ...
                $error = 'Este portal es exclusivo para supervisores.';
                ...
                $_SESSION['supervisor_portal_uid']    = (int)$user['id'];
                $_SESSION['supervisor_portal_nombre'] = $user['nombre'];
                ...
                try {
                    ...
                    error_log('update ultimo_acceso supervisor: ' . $e->getMessage());
                }
                $redirect = $_SESSION['supervisor_redirect_after_login'] ?? 'dashboard.php';
                unset($_SESSION['supervisor_redirect_after_login']);
                header('Location: ' . ($redirect ?: 'dashboard.php'));
                exit();
            }
        } catch (PDOException $e) {
            error_log('supervisor/index.php PDO: ' . $e->getMessage());
...
<head>
    ...
    <title>Portal Supervisor — <?php echo htmlspecialchars($config['nombre_empresa']); ?></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-login:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(109,40,217,.3); }
